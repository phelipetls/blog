---
title: Type-safe polymorphic components in React
date: 2023-04-01
...
accepts the element type and props as argument. With this, we can make our
generic return all props that element type accepts plus all props these
additional specific component props:
...
Notice that we use `Omit` to eliminate any possible conflicting props between
the result of `PropsOf` and `TProps`. In this case, we choose `TProps` to take
precedence.
