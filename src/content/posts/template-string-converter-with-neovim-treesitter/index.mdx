...
import PlaygroundDemo from './_playground-demo.png'
import InsertLeaveDemo from './_insertleave.webm'
import TextChangedDemo from './_textchanged.webm'
...
```lua title="~/.config/nvim/lua/template-string-converter.lua"
local M = {}
...
<Image src={PlaygroundDemo} alt={`A screenshot of neovim's syntax tree representation of a JavaScript program, as implemented in nvim-treesitter's playground plugin.`} />
...
```vim title="~/.config/nvim/after/ftplugin/javascript.vim"
if has('nvim')
...
<Video src={InsertLeaveDemo} caption="Using with InsertLeave event." />
<Video src={TextChangedDemo} caption="Using with TextChanged event." />
