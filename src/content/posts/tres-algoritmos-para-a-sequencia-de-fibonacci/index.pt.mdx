...
import GoldenRatio from './_golden-ratio.png'
import Fibonacci from './_fibonacci.png'
import EigenVectors from './_evectors.gif'

<Image alt="A espiral dourada" src={GoldenRatio} />
...
<Image alt="Uma ilustração do algoritmo recursivo para a sequência de Fibonacci" src={Fibonacci} />
...
<YouTube id="13r9QY6cmjc" />

Finalmente, matrizes. Comecemos primeiro com a equação básica, $ F*n = F*{n-1} + F\_{n-2} $.

Mas, para termos um sistema de equações lineares, precisamos de mais uma equação
pelo menos. A escolha mais simples é: $ F*{n-1} = F*{n-1} + 0\cdot F\_{n-2} $. O que, em linguagem de matrizes, pode ser escrito como:
...
Assim, segue diretamente que $ u_1 = Au_0 $. E, disso, que $ u_2 = Au_1 = A
\cdot A \cdot u_0 = A^2u_0 $. Logo, a fórmula geral é:
...
<Image alt="Animação de como funcionam os Eigenvectors, mostrando que eles não mudam de direção mesmo após uma transformação matricial" caption="Ilustração de como funcionam os Eigenvectors" src={EigenVectors} />
...
autovetor. Note que, por isso, o autovetor pode encolher (se $ 0 < \lambda < 1 $), ou aumentar (se $ \lambda \> 1 $), ou apontar para a direção oposta (se $ \lambda < 0 $) quando transformado.
...
Daí que, tratando $ S $ como uma matriz de autovetores nas colunas e $ \Lambda $ como uma matriz de autovalores na diagonal e zeros em todo o resto,
podemos afirmar que
...
mais especificamente, como uma combinação linear dos autovetores, $ Sc = u_0 $, onde c é o vetor de coeficientes que resolve esse sistema de equações, $ c = S^{-1}u_0 $. E, com isso, nós temos tudo que precisamos porque
